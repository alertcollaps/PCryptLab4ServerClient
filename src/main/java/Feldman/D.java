...
import Feldman.Client;
import Feldman.Feldman_scheme;

import java.math.BigInteger;
import java.util.Scanner;

public class D {
    static String preString = "[FELDMAN]:";
    ...
    public static void main(String[] args) {
        ...
        int[] xi = feldman_scheme.initialiseClients(n);
        for (int i = 0; i < n; i++){ //Заполнение клиентов
            int xInd = xi[i];
            clients[i] = new Client(xInd, feldman_scheme.getP(), feldman_scheme.getCheckNumbers(), feldman_scheme.getG());
            clients[i].setYi(feldman_scheme.getYi(xInd), feldman_scheme.getZi(xInd));
        }
        ...
        while (num != 3){
            ...
            log.append(preString + "Равенство K == shamir.K:" + K.equals(feldman_scheme.getK()) + "\n");
