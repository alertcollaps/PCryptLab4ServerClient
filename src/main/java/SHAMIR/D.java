...
public class D {
    ...
    public static void main(String[] args) {
        ...
        Shamir_scheme shamir_scheme = new Shamir_scheme(t);
        shamir_scheme.chooseAi();
        log.append(preString + "Shamir secret key:" + shamir_scheme.getK() + "\n");
        BigInteger P = shamir_scheme.getP();
        log.append(preString + "Shamir P:" + shamir_scheme.getP() + "\n");
        int[] xi = shamir_scheme.initialiseClients(n);
        for (int i = 0; i < n; i++){ //Заполнение клиентов
            int xInd = xi[i];
            clients[i] = new Client(xInd, shamir_scheme.getYi(xInd));
        }
        ...
        while (num != 3){
            ...
            log.append(preString + "Равенство K == shamir.K:" + K.equals(shamir_scheme.getK()) + "\n");
